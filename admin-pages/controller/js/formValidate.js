...
function formClothes_Validate() {
    $flag = true;
    pattern = /^[\p{L}\s\d]*[\p{L}\s\d]*[\p{L}\s\d]*$/u;
    $title = document.forms['itemClothes']['title'].value;
    $decripsion = document.forms['itemClothes']['decripsion'].value;
    $price = document.forms['itemClothes']['price'].value;
    if ($title == "" || $title == null) {
        document.getElementById('title-warning').innerHTML = "Tên sản phẩm không được để trống"
        $flag = false;
